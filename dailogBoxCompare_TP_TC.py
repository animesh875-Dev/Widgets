...
data_governance_TC = config["data_governance_TC"] # Assuming max allowed test cases is 100, replace with actual value if needed
...
def fetch_oslc_details(project_area_uuid):
    ...
    try:
        ...
        log_message_to_file(f"Error parsing XML response for Project Area UUID {project_area_uuid}: {e}")
    except requests.exceptions.RequestException as e:
        print(f"Error fetching OSLC details for Project Area UUID {project_area_uuid}: {e}")
        log_message_to_file(f"Error fetching OSLC details for Project Area UUID {project_area_uuid}: {e}")
...
def fetch_test_case_count(project_area_uuid, oslc_id):
    """
    Fetches the test case count for a given project area and OSLC ID.
    """
    test_case_api_url = f"{server_url}/qm/oslc_qm/contexts/{project_area_uuid}/resources/com.ibm.rqm.planning.VersionedTestCase"
    try:
        print(f"Fetching test case count for {project_area_uuid} and OSLC ID {oslc_id}...")
        response = requests.get(test_case_api_url, auth=(username, password), verify=False)
        response.raise_for_status()

        root = ET.fromstring(response.text)
        total_count_element = root.find(".//oslc:totalCount", {"oslc": "http://open-services.net/ns/core#"})
        if total_count_element is not None:
            return int(total_count_element.text)
    except ET.ParseError as e:
        print(f"Error parsing XML response for Test Case Count: {e}")
        log_message_to_file(f"Error parsing XML response for Test Case Count: {e}")
    except requests.exceptions.RequestException as e:
        print(f"Error fetching Test Case Count: {e}")
        log_message_to_file(f"Error fetching Test Case Count: {e}")
...
#         test_case_count = fetch_test_case_count(project_area_uuid, selected_oslc_id)

#         # Display message for Test Plan and Test Case count
#         message = (
...
#             f"Remaining Test Plans: {data_governance_TP - test_plan_count}\n\n"
...
#             f"Remaining Test Cases: {data_governance_TC- test_case_count}"
#         )

#         # Show info message
#         messagebox.showinfo("Test Plan and Test Case Validation", message)

#         # Log the message to a file
#         log_message_to_file(message, selected_project_area)
...
def on_validate_data_click():
    ...
    if selected_project_area:
     ...
     # Include Project Area and Stream Name in the message
        ...
        # Show the appropriate message in a popup
        messagebox.showinfo("Test Plan and Test Case Validation", message)
